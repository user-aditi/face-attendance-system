...
from tkcalendar import DateEntry # <-- IMPORT THE NEW CALENDAR WIDGET

# (The AddStudentWindow class remains unchanged)
class AddStudentWindow(ctk.CTkToplevel):
    # ... (code for this class is the same as before, no changes needed)
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.geometry("1100x600") # Increased size for better report view
    ...
    # (create_dashboard_frame and create_manage_student_frame remain unchanged)
    ...
    def create_reports_frame(self):
        ...
        # Define headings and add the sorting command to each
        for col in columns:
            self.log_tree.heading(col, text=col.replace("_", " ").title(), command=lambda c=col: self.sort_treeview_column(c, False))
    ...
    # (refresh_student_list, show_dashboard_frame, and show_manage_frame remain unchanged)
    def refresh_student_list(self):
        for widget in self.student_list_frame.winfo_children(): widget.destroy()
        students = self.db_manager.get_all_students()
        if not students: ctk.CTkLabel(self.student_list_frame, text="No students found.").pack(pady=20)
        else:
    ...
    ### --- MODIFIED AND NEW METHODS FOR REPORTS --- ###
    def show_reports_frame(self):
        self.dashboard_frame.grid_forget()
        self.manage_student_frame.grid_forget()
        self.reports_frame.grid(row=0, column=1, sticky="nsew")
        self.search_daily_report() # Show today's report by default

    def clear_filters(self):
        """NEW: Clears the search box and resets the date to today."""
    ...
    def sort_treeview_column(self, col, reverse):
        """This method enables sorting by clicking column headers."""
        data_list = [(self.log_tree.set(k, col), k) for k in self.log_tree.get_children('')]
        
        # A simple numeric sort for ID, otherwise string sort
        try:
        ...
        try:
            with open(filepath, "w", newline="", encoding="utf-8") as f:
                ...
                for item_id in self.log_tree.get_children():
                    writer.writerow(self.log_tree.item(item_id)["values"])
            messagebox.showinfo("Success", f"Data exported to {filepath}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to export data: {e}")
            
    # (open_add_student_window and other button commands remain unchanged)
